...
export function EditBazaarDialog({
  id,
  amount,
  entryDate,
  description,
  ...
  const [state, action, pending] = useActionState(updateBazaarEntry, undefined);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <Button variant="ghost" size="icon-sm" onClick={() => setOpen(true)}>
        ...
        <span className="sr-only">Edit bazaar entry</span>
      </Button>
      <DialogContent>
        <DialogHeader>
        ...
        <form action={action} className="flex flex-col gap-4 p-4 pt-2">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <Input id={`date_${id}`} name="entry_date" type="date" defaultValue={entryDate} required />
            </div>
